...
local methods = {
	...
	SetColumns = function(self, columns)
		...
		for i = 1, #columns do
			...
			if not columnHeadFrame.columns[i] then
				...
				columnFrame.arrow =
					self.stdUi:Texture(columnFrame, 8, 8, [[Interface\Buttons\UI-SortArrow]])
	...
	IsRowVisible = function(self, rowIndex)
		return (rowIndex > self.offset and
			rowIndex <= (self.numberOfRows + self.offset))
	end,
	-------------------------------------------------------------
		...
		if shouldShow then
			...
			elseif format == "icon" then
				if cellFrame.texture then
					...
					cellFrame.texture =
						table.stdUi:Texture(cellFrame, iconSize, iconSize, value)
...
local ScrollTableOnVerticalScroll = function(self, offset)
	...
	scrollTable:DoVerticalScroll(
		offset,
		scrollTable.rowHeight,
		ScrollTableUpdateFn
	)
end
